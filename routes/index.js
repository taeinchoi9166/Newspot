...
router.get('/', function(req, res, next) {
  if(req.session.userToken!==undefined){
    ...
    return false;
      ...
      let result = request.post({uri:authURL},(err,resp,body)=>{
          // console.log(result);
          ...
          try{
              ...
              if(response!==undefined&&response.access_token!==undefined){
                  ...
                  req.session.cookies.maxAge = response.expires_in;
              ...
              let renderPage = '';
              if(nowPage!=='index') renderPage = 'spotify/'+nowPage;
              res.redirect('/'+renderPage);
          }catch(err){
              res.render('index',{res:undefined,result:{msg:err.message}});
          }
      });
...
router.get('/release',(req,res)=>{
  ...
  if(req.session.userToken===undefined){
    ...
    res.redirect('/?nowPage=release');
      ...
      let result = request.post('http://localhost:3000/api/release',{form:{token:req.session.userToken.access_token}},(err,resp,body)=>{
          console.log(userToken);
    ...
    res.redirect('/?nowPage=recommend');
      ...
      function getGenreList() { //시드 목록 받기
          return new Promise((resolve, reject) => {
              let result = request.post('http://localhost:3000/api/recommend/getseed', {form: {token: req.session.userToken.access_token}}, (err, resp, body) => {
      ...
      function isValidSeed() { //시드 값 검사
          return new Promise((resolve, reject) => {
              let seedList = request.post('http://localhost:3000/api/recommend/chkseed', {
                  form: {
                      token: req.session.userToken.access_token,
                      data: genres
                  }
              }, (err, resp, body) => {
                  if (err || resp.body === undefined) {
      ...
      function getRecommendList() { //추천 리스트 받기
          return new Promise((resolve, reject) => {
              let recommendList = request.post('http://localhost:3000/api/recommend/list', {
                  form: {
                      token: req.session.userToken.access_token,
                      data: genres
                  }
...
router.get('/search',(req,res)=>{
  ...
  if(req.query!==undefined&&req.query.keyword!==undefined){
    let result = request.post('http://localhost:3000/api/search',{form:{token:req.session.userToken.access_token,keyword:req.query.keyword,min_popularity:req.query.min_popularity,criteria_date:req.query.criteria_date}},(err,resp,body)=>{
      let response = undefined;
      try{
        response = JSON.parse(body);
      }catch(e){
